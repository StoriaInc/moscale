...
{
	//override def jColl(implicit mongo : Mongo) = mongo.

  import EntityId.Field.Id
  import Value.{Map => VMap, _}

	def collection(implicit mongo : Mongo) =
		mongo.getCollection[T]


  def drop()(implicit mongo : Mongo) {
    collection.drop()
  }


	def find(implicit mongo : Mongo) : Cursor[T] =
		collection.find[T]


	def find(query : DBObject, fields : DBObject)(implicit mongo : Mongo) : Cursor[T] =
		collection.find[T](query, fields)


	def find(query : Value.Map, fields : Map[String, Boolean] = null)(implicit mongo : Mongo) : Cursor[T] =
		collection.find[T](query, fields)
  ...
  def findById(id: ObjectId)(implicit mongo: Mongo) : Option[T] = collection.findById[T](id)


  def save(entity : T)(implicit mongo : Mongo) : WriteResult = collection.save(entity)


  def insert(entity : T)(implicit mongo : Mongo) : WriteResult = collection.insert(entity)
...
//    collection.update(query, obj, upsert, multi)
  ...
  (implicit th : TypeHandler[T], mongo : Mongo)
  : Option[T] =
    collection.findAndModify[T](query, update, order = order, fields = fields, upsert = upsert, returnNew = returnNew)


  def remove(query : DBObject)(implicit mongo : Mongo) : WriteResult =
    collection.remove(query)


  def remove(query : Value.Map)(implicit mongo : Mongo) : WriteResult =
    collection.remove(query)


  def remove(entity : T)(implicit mongo : Mongo) : WriteResult =
    collection.remove(entity)


  def aggregate(query: VMap, additionalQueries: VMap*)(implicit mongo: Mongo) =
  ...
  def count(implicit mongo : Mongo) = collection.count

  def init()(implicit mongo : Mongo) {}
}
